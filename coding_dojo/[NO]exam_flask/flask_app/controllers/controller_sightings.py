...
# @app.route("/add_skeptic_sighting/<int:id>/<int:id>", methods=["POST"])  
# def add_skeptic_sighting(id,id2):
#     data = {
#     "user_id" : id, #gets from <select name="authors_id">, returns value = {{author.id}} 
#     "sighting_id": id2
#     }
#     Sighting.add_to_sighting_skeptics(data) #returns a dojo with a list of ninjas
#     return redirect(f'/show/{id2}') #redirect goes to route, render_template shows html page

@app.route("/add_skeptic_sighting/<int:id>", methods=["POST"])  
def add_skeptic_sighting(id):
    data2 = {
    "user_id" : session['user_id'], #gets from <select name="authors_id">, returns value = {{author.id}} 
    "sighting_id": id
    }
    Sighting.add_to_sighting_skeptics(data2) #returns a dojo with a list of ninjas
